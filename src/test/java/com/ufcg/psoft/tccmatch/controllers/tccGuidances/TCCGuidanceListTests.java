...
class TCCGuidanceListTests extends TCCGuidanceTests {
  ...
  void listWithoutFilters() throws Exception {
    makeListTCCGuidancesRequest(coordinatorToken, Optional.empty(), Optional.empty())
  ...
  void listByPeriod() throws Exception {
    makeListTCCGuidancesRequest(coordinatorToken, Optional.of(periods.get(0)), Optional.empty())
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.[0].id", is(tccGuidances.get(0).getId().intValue())))
      .andExpect(jsonPath("$.[1].id", is(tccGuidances.get(1).getId().intValue())));
  }
  ...
  void listByIsFinished() throws Exception {
    makeListTCCGuidancesRequest(coordinatorToken, Optional.empty(), Optional.of(false))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.[0].id", is(tccGuidances.get(0).getId().intValue())));

    makeListTCCGuidancesRequest(coordinatorToken, Optional.empty(), Optional.of(true))
      .andExpect(status().isOk())
  ...
  void listByPeriodAndIsFinished() throws Exception {
    makeListTCCGuidancesRequest(coordinatorToken, Optional.of(periods.get(1)), Optional.of(true))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.[0].id", is(tccGuidances.get(2).getId().intValue())));
  }

  private ResultActions makeListTCCGuidancesRequest(
    String token,
