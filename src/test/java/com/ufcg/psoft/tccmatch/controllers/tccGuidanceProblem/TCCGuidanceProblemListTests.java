...
public class TCCGuidanceProblemListTests extends TCCGuidanceProblemTests {
  ...
  void beforeEach() {
    ...
    CreateTCCGuidanceProblemDTO tccGuidanceProblemDTO = new CreateTCCGuidanceProblemDTO(
        ...
        description,
        tccGuidance.getId());
    tccGuidanceProblems.add(tccGuidanceProblemService.createTCCGuidanceProblem(tccGuidanceProblemDTO, student));

    tccGuidanceProblemDTO = new CreateTCCGuidanceProblemDTO(
        category,
        description,
        tccGuidance.getId());
    tccGuidanceProblems.add(tccGuidanceProblemService.createTCCGuidanceProblem(tccGuidanceProblemDTO, professor));
  }
  ...
  void listTCCGuidanceProblemsGroupedByStudentsAndProfessors() throws Exception {
    makeListTCCGuidanceProblemsRequest(coordinatorToken)
        ...
        .andExpect(jsonPath("$.studentsTccGuidanceProblems.[0].id", is(tccGuidanceProblems.get(0).getId().intValue())))
        .andExpect(
            jsonPath("$.professorsTccGuidanceProblems.[0].id", is(tccGuidanceProblems.get(1).getId().intValue())));
  }

  private ResultActions makeListTCCGuidanceProblemsRequest(
      String token) throws Exception {
    return mvc.perform(
        authenticated(
            get("/api/tcc-guidance-problems"),
            token));
  }
